<script lang="ts">
  ...
  if (browser)
    currentTheme =
      ...
      (window.matchMedia('(prefers-color-scheme: light)').matches ? theme?.[1].name : theme[0].name ?? theme[0].name)
</script>
    ...
    <div in:fly={{ x: -50, duration: 300, delay: 300 }} out:fly={{ x: -50, duration: 300 }} class="navbar">
      ...
      <div class="navbar-end">
        ...
        <div id="change-theme" class="dropdown dropdown-end">
          ...
          <ul
            tabindex="0"
            class="flex flex-nowrap shadow-2xl menu dropdown-content bg-base-100 text-base-content rounded-box w-16 p-2 gap-2 overflow-y-auto max-h-[21.5rem]"
            class:hidden={!pin}>
